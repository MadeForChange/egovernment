...
if (isset($_POST['update_settings'])) {
		...
		if($_POST['hash'] == $_SESSION[$elhash]){
			...
			$q_modal = $db->escape_value($_POST["q_modal"]);
...
<div class="row">
	...
	<div class="col-md-3 ">
		<h4><i class="fa fa-wrench"></i>&nbsp;&nbsp;<?php echo $lang['admin-title']; ?></h4>
		<hr>
		<ul class="feed-ul ">
	...
	<div class="col-md-9">
		...
		<div class="tab-content">
				...
				<h3 class="page-header"><?php echo $lang['admin-general-title']; ?></h3>
					...
					<div class="row">
						<div class="col-md-6">
							...
							<div class="form-group">
								...
								<label for="site_logo"><?php echo $lang['admin-general-site-logo']; ?></label>
									...
									?>
										<img src="<?php echo WEB_LINK."public/".$file->image_path(); ?>" class='img-responsive'/><br><div class='col-xs-6'><input type='checkbox' name='change_logo' value= '1' > Change Logo</div><div class='col-xs-6'><input type='checkbox' name='delete_logo' value= '1' > Delete Logo</div><br><br>
									<?php
										} else {
									...
									<small>Max. Width: 140px, Max. Height: 30px</small><br/>
								<br>
						...
						<div class="col-md-6">
							...
							<div class="form-group">
								<label for="q_modal"><?php echo $lang['admin-general-posting-q_modal']; ?></label>&nbsp;&nbsp;
								<select id="q_modal" name="q_modal" class="form-control select2" style="width:200px"> 
									<option value="1" <?php if(isset($settings['q_modal']) && $settings['q_modal']== '1') { echo ' selected'; } ?> >Enable</option>
									<option value="0" <?php if(isset($settings['q_modal']) && $settings['q_modal']== '0') { echo ' selected'; } ?>>Disable</option>
								</select><br><br>
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<div class="controls">
									...
									<img src="<?php echo WEB_LINK.'public/img/topic.png'; ?>" class="img-polaroid img-circle" style="float:<?php echo $lang['direction-left']; ?>; padding:5px; margin-<?php echo $lang['direction-right']; ?>:10px; width:64px; height:64px" id="img1">
									<div style="height:64px; padding-top: 12px;width:200px;float:<?php echo $lang['direction-left']; ?>">
										<input class="text-input " type="file" name="upload_files[]" id="img1_upl"/><br/>
									</div>
			...
			<h3><?php echo $lang['admin-section-admanager']; ?>&nbsp;&nbsp;<a href='<?php echo $url_mapper['admin/'] . '?section=admanager&type=new'; ?>' class="btn btn-sm btn-primary"><i class='fa fa-plus'></i> <?php echo $lang['btn-add']; ?></a></h3>
				<?php if($section == 'admanager' && isset($_GET['type']) && $_GET['type'] == 'new' ) { 
				?>
				<hr>
				<form id="form-validation" action="<?php echo $url_mapper['admin/']; ?>" method="post" class="">
						...
						<div class="col-md-6">
						<div class="form-group">
							<label for="link" class="control-label">Ad Link</label>
						...
						<div class="col-md-6">
						<div class="form-group">
							<label for="expiry" class="control-label">Ad Location</label>
						...
						<div class="col-md-6">
						<div class="form-group">
							<label for="expiry" class="control-label">Expire after</label>
					...
					<center>
						...
						<a href='<?php echo $url_mapper['admin/'] . '?section=admanager'; ?>' class="btn btn-danger" ><?php echo $lang['btn-cancel']; ?></a>
					</center>		
					
					</form>
				<?php
				} elseif($section == 'admanager' && isset($_GET['type']) && $_GET['type'] == 'edit'  && isset($_GET['id']) && is_numeric($_GET['id']) && isset($_GET['hash'])) {
				...
				<?php
				} elseif($section == 'admanager' && isset($_GET['type']) && $_GET['type'] == 'delete'  && isset($_GET['id']) && is_numeric($_GET['id']) && isset($_GET['hash'])) {
					$id = $db->escape_value($_GET['id']);
				...
				<table class="table table-hover table-bordered custom_table">
                      ...
                      <tbody>
                        <?php 
							...
							if($current_user->can_see_this("admanager.update",$group)) {
								$edit = "<a href=\"{$url_mapper['admin/']}?section=admanager&id={$obj->id}&type=edit&hash={$random_hash}\" class='btn btn-sm btn-icon btn-rounded btn-primary' data-toggle='tooltip' data-placement='top' title='Edit' data-original-title='Edit'  ><i class='fa fa-pencil'></i></a>";
							} else {
							...
							if($current_user->can_see_this("admanager.delete",$group)) {
								$delete = "<a href=\"{$url_mapper['admin/']}?section=admanager&id={$obj->id}&type=delete&hash={$random_hash}\" class='btn btn-sm btn-icon btn-rounded btn-danger' data-toggle='tooltip' data-placement='top' title='Delete' data-original-title='Delete'   onclick=\"return confirm('Are you sure you want to delete this ad?');\" ><i class='fa fa-times'></i></a>";
							} else {
...
$('.approve-machine').on('click' , '.approve-item' , function() {
	...
	if(confirm('Are you sure you want to approve this item?')) {
		...
		$.post("<?php echo WEB_LINK; ?>public/includes/one_ajax.php?type=approve", {id:id, data: $(this).data('obj') , hash:'<?php echo $random_hash; ?>'}, function(){});
	}
});
$('.approve-machine').on('click' , '.reject-item' , function() {
	...
	if(confirm('Are you sure you want to reject this item?')) {
		$(this).parent().parent().parent().hide();
		$.post("<?php echo WEB_LINK; ?>public/includes/one_ajax.php?type=reject", {id:id, data: $(this).data('obj') , hash:'<?php echo $random_hash; ?>'}, function(){});
	}
});
...
$('.approve-machine').on('click' , '.approve-report-item' , function() {
	...
	if(confirm('<?php echo $lang['admin-reports-approve_report-alert']; ?>')) {
		$(this).parent().parent().parent().hide();
		$.post("<?php echo WEB_LINK; ?>public/includes/one_ajax.php?type=approve-report", {id:id, data: $(this).data('obj') , report_id: $(this).data('report_id'),  hash:'<?php echo $random_hash; ?>'}, function(){});
	}
});
$('.approve-machine').on('click' , '.reject-report-item' , function() {
	...
	if(confirm('<?php echo $lang['admin-reports-reject_report-alert']; ?>')) {
		$(this).parent().parent().parent().hide();
		$.post("<?php echo WEB_LINK; ?>public/includes/one_ajax.php?type=reject-report", {id:id, data: $(this).data('obj') , report_id: $(this).data('report_id') , hash:'<?php echo $random_hash; ?>'}, function(){});
	}
});
